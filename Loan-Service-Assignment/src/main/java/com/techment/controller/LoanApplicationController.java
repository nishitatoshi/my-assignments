...
import com.techment.exception.LoanApplicationAlreadyExistsException;
...
public class LoanApplicationController {
	...
	public ResponseEntity<LoanApplication> saveLoanApplication(@RequestBody LoanApplication loanApplication,
		...
		try {
			...
			throw new LoanApplicationAlreadyExistsException("Already Exist");
		}
	}
	...
	public ResponseEntity<LoanApplication> approveLoanApplication(@PathVariable long loanId,
		...
		try {
			...
			throw new LoanApplicationNotFoundException("loan id not found for approve");
							}
	}
	...
	public ResponseEntity<LoanApplication> rejectLoanApplication(@PathVariable long loanId,
		...
		try {
			Optional<UserDto> user = iUserService.findByToken1(token);
			if (user.isEmpty()) {
				throw new UnAuthorisedException("incorrect Token");
			}
			...
			throw new LoanApplicationNotFoundException("loan id  not found to reject");
		}
	}
